...
func (e *EvaluationService) initScales() {
	for i := range e.scale {
		e.scale[i] = 1
	}
...
func (e *EvaluationService) evaluateCore(p *Position) int {
	...
	/*e.add(fKingAttack,
	Min(150, battackTot*Max(1, battackTot-5))-
		Min(150, wattackTot*Max(1, wattackTot-5)))*/

	var danger = battackCount - 1
	if bq == 0 {
		danger -= 6
	}
	if danger > 0 {
		e.add(fKingAttack, danger)
	}
	danger = wattackCount - 1
	if wq == 0 {
		danger -= 6
	}
	if danger > 0 {
